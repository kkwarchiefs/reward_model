...
class RerankerTrainer(Trainer):
    def evaluate(
        ...
        for ins, prediction in zip(self.eval_dataset, output.predictions):
            ...
            print(response, prediction, ins[0]['level_label'], len(response),  sep='\t', file=fout)
        fout.close()
...
class Reranker(nn.Module):
    ...
    def forward(self, **batch):
        ...
        # print(pooling_hidden_states, pooling_hidden_states.shape)
        # print( torch.sum(batch['generation_mask'], axis=1),  torch.sum(batch['generation_mask'], axis=1).shape)
        ...
        if self.training:
            ...
            target_label = torch.zeros(scores.shape[0], dtype=torch.long).to(scores.device)
...
class RerankerPair(nn.Module):
    ...
    def forward(self, **batch):
        ...
        if self.training:
            ...
            # target_label = torch.zeros(scores.shape[0], dtype=torch.long).to(scores.device)
            loss = -nn.functional.logsigmoid(scores[:, 0] - scores[:, 1]).mean()
            # loss = self.cross_entropy(scores, target_label)
...
class SequenceClassify(nn.Module):
    ...
    def forward(self, **batch):
        ...
        if 'generation_mask' in batch:
            batch['generation_mask'] = batch['generation_mask'].to(self.hf_model.device)
        if 'level_label' in batch:
            batch.pop('level_label')
        # batch = batch.to(self.accelerator.device)
        # print({k: v.shape for k, v in batch.items()})
        # print({k: v for k, v in batch.items()})
        ranker_out = self.hf_model(**batch)
        # print(ranker_out.hidden_states[:, 0, :])
        ...
        if self.training:
            scores = logits.view(
                -1,
                self.data_args.rank_list_size + 1
            )
            target_label = torch.zeros(scores.shape[0], dtype=torch.long).to(scores.device)
            loss = self.cross_entropy(scores, target_label)
            # loss = -nn.functional.logsigmoid(scores[:, 0] - scores[:, 1]).mean()
