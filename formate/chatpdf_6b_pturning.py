...
from transformers import BertTokenizer, AutoTokenizer, AutoModelForSeq2SeqLM, AutoConfig
import os
import sys
...
CHECKPOINT_PATH = "/search/ai/jamsluo/chatglm_6b/output/rc-1536-chatglm-6b-pure-256-1e-3-04-25-20-53/checkpoint-2000/"
device = "cuda:3"
tokenizer_glm = AutoTokenizer.from_pretrained(CHECKPOINT_PATH, trust_remote_code=True, truncation_side="left")
...
model = AutoModelForSeq2SeqLM.from_pretrained('/search/ai/pretrain_models/chatglm-6b/', config=config, trust_remote_code=True)
...
model.transformer.prefix_encoder.float().to(device)
model = model.eval()
