...
def sample_sequence(model, tokenizer, context_tokens, context_length, args, device, mems=None, end_tokens=None, continue_num=0):
    ...
    while counter < args.out_seq_length:
        ...
        if args.num_beams > 1:
            ...
            prev = torch.topk(log_probs, 1)[1][0]
...
def read_context_from_file(tokenizer, args, output, single_test_data):
    terminate_runs, skip_run = 0, 0
    if mpu.get_model_parallel_rank() == 0:
        raw_text = single_test_data
        raw_text = raw_text  + " [回答]"
        if not raw_text:
            print('Prompt should not be empty!')
        #generation_mask = '[gMASK]' if args.task_mask else '[MASK]'
        ...
        if args.block_lm:
            ...
            if not raw_text.endswith('[gMASK]'):
                context_tokens = context_tokens + [tokenizer.get_command('eos').Id]
        context_tokens = context_tokens[-args.seq_length:]
        print (context_tokens)
        context_length = len(context_tokens)
    else:
        context_length = 0
...
def generate_samples(model, tokenizer, args, device):
    ...
    test_datas = open("./test_datas/ge_input2.csv")
    result_file = open(f"./test_datas/sample-{datetime.now().strftime('%m-%d-%H-%M')}.txt", "w")
    with torch.no_grad(), open(output_path, "w") as output:
        for single_test_data in test_datas:
            query_input = single_test_data.strip().split('\t')[2]
            torch.distributed.barrier(group=mpu.get_model_parallel_group())

            terminate_runs, raw_text, context_tokens_tensor, context_length = read_context_from_file(tokenizer, args, output, query_input)
            if terminate_runs == 1:
                return
            ...
            if mpu.get_model_parallel_rank() == 0:
                ...
                result_file.write(trim_decode_tokens + "\n")
