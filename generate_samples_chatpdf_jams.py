...
def get_batch(context_tokens, device, args):
    ...
    if args.block_lm:
        ...
        if not args.no_block_position:
            ...
            block_position_ids = torch.arange(cut_num, device=device, dtype=torch.long).view(1,cut_num).repeat(1024, 1).transpose(0, 1).contiguous().view(-1)[0:tokens.size(1)]
...
def read_context(tokenizer, args, input_data):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        while True:
            ...
            context_tokens = context_tokens[-1020:]
...
def generate_samples(model, tokenizer, args, device):
    model.eval()
    #chatpdf = ChatPdf(glm_inference, content_type="PDF file")
    chatpdf = ChatPdf(glm_inference)
    while True:
        path = input("请输入文件：")
